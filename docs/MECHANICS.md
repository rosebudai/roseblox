...
`paused:true` freezes physics and fixed callbacks, clears held input on entry and prevents paused-time catch-up. An enabled desktop controller that loses focus/capture also pauses the world automatically, including fixed callbacks. Reflect `player.enabled && !player.active` in the resume UI; gate timers outside fixed callbacks on `.active` too. Advanced hosts managing multiple input surfaces can set `autoPause:false`. `createMechanics({gravity:[0,-9.81,0],fixedTimeStep:1/60,maxSubSteps:8,maxFrameDelta:.25,maxCcdSubsteps:4,interpolate:true})` permits overrides. `getDiagnostics()` reports bodies, contacts, frames, fixedSteps, simulatedSeconds, droppedSeconds and disposed. Call `mechanics.dispose()` on teardown; separately dispose your art, renderer, listeners and asset helpers.
